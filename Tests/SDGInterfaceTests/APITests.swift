...
final class APITests: ApplicationTestCase {
  ...
  func testLegacyView() {
    if #available(watchOS 6, *) {
        ...
        class Legacy: LegacyView, SDGInterface.View {
          // #workaround(Swift 5.7.2, The next #if is redundant, but for compiler bug.)
  ...
  func testMenuEntry() {
    if #available(tvOS 13, watchOS 6, *) {
  ...
  func testTextField() {
    ...
    if #available(macOS 10.15, tvOS 13, iOS 13, watchOS 6, *) {
      #if canImport(SwiftUI)
        let testBody: Bool
        // #workaround(Swift 5.7.2, SwiftUI would be a step backward from AppKit or UIKit without the ability to get the selected text for menu items like “Show Character Information”.)
        #if !canImport(AppKit) && !(canImport(UIKit) && !os(watchOS))
          testBody = true
        #else
          testBody = false
